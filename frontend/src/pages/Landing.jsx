...
export default function Navbar() {
  ...
  // Track scroll position for transparent vs. frosted glass background[cite: 15]
  useEffect(() => {
    const handleScroll = () => {
  ...
  // Close mobile menu and unlock body scroll on route change[cite: 15]
  useEffect(() => {
    setIsOpen(false);
    document.body.style.overflow = 'unset';
  }, [location]);

  // Lock background scrolling when mobile menu is open[cite: 15]
  const toggleMenu = () => {
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex items-center justify-between">
          ...
          {/* Brand Logo with Category Descriptor */}
          <Link to="/" className="flex items-center gap-3 group z-50">
            <div className="relative flex items-center justify-center p-1.5 rounded-2xl bg-white/80 backdrop-blur-sm border border-slate-200/80 shadow-sm transition-all group-hover:scale-105">
              <img 
                src={logo} 
